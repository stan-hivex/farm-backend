...
- Do not commit `.env` or any real credentials to source control.
- Use `.env.example` for local development only.
- In production, use a secrets manager or environment variables injected by your deployment platform.
- If using AWS Secrets Manager, set `USE_AWS_SECRETS_MANAGER=true`, `AWS_SECRETS_MANAGER_SECRET_ID`, and `AWS_REGION`.
