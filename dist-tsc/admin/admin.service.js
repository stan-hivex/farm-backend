...
let AdminService = class AdminService {
    constructor(prisma, escrowService, notifications, withdrawService, currencyConversionService) {
        this.prisma = prisma;
    ...
    async getDashboardStats() {
        const [totalUsers, totalMerchants, activeEscrows, txVolume, pendingKyc, pendingPayouts] = await Promise.all([
    ...
    async resolveDispute(escrowId, adminId, dto) {
        ...
        if (dto.winner === 'seller')
            await this.escrowService.executeRelease(escrow);
        else
            await this.escrowService.executeRefund(escrow);
    ...
    async approveMerchant(merchantId, adminId, dto) {
        const merchant = await this.prisma.merchants.update({
            where: { id: merchantId },
            data: { status: dto.status, approved_by: adminId, approved_at: new Date() },
        });
        await this.prisma.audit_logs.create({
            data: {
    ...
    async updateFee(feeId, value, adminId) {
        ...
        if (Number.isNaN(numericValue)) {
            throw new common_1.BadRequestException('Fee value must be numeric');
        }
    ...
    async processPayout(payoutId, adminId, status) {
        const payout = await this.prisma.merchant_payouts.update({
            where: { id: payoutId },
            data: { status: status, processed_by: adminId, processed_at: new Date() },
        });
        return { data: payout, message: `Payout marked ${status}` };
    }
    async sendNotification(adminId, dto) {
    ...
    async broadcastNotification(adminId, dto) {
        ...
        await Promise.all(users.map((user) => this.notifications.sendNotification(user.id, {
    ...
    async getAdminAnalytics() {
        const [totalUsers, totalEscrows, totalTransactions, pendingKyc, pendingPayouts, totalSecurityEvents] = await Promise.all([
            this.prisma.users.count({ where: { is_deleted: false } }),
            this.prisma.escrow_contracts.count({ where: {} }),
            this.prisma.transactions.count({ where: {} }),
            this.prisma.kyc_documents.count({ where: { status: 'pending' } }),
    ...
    async updateCurrencyRate(usdKesRate, adminId) {
    ...
    async updateExchangeRates(rates, adminId) {
        const upserted = [];
    ...
    async getUserActivityLog(userId, query) {
        const { skip, take, page, limit } = (0, pagination_util_1.paginationParams)(query.page, query.limit);
    ...
    async getSecurityStats() {
        const [totalEvents, criticalEvents, highEvents, failedLogins, tokenThefts, suspendedAccounts] = await Promise.all([
            this.prisma.security_events.count(),
    ...
    async reviewKyc(kycDocId, adminId, dto) {
        ...
        await this.prisma.users.update({
            ...
            data: { kyc_status: dto.status, kyc_level: kycLevel },
        });
        await this.prisma.audit_logs.create({
            data: {
                user_id: adminId,
    ...
    async createSuperadmin(dto, adminId) {
        ...
        const user = await this.prisma.$transaction(async (tx) => {
            ...
            await tx.audit_logs.create({
                data: {
                    ...
                    entity_type: 'users',
                    entity_id: u.id,
                    new_values: { first_name: u.first_name, last_name: u.last_name, username: u.username, role: 'admin' },
                },
            });
            return u;
        });
    ...
    async updateUser(userId, dto, adminId) {
        ...
        if (dto.is_suspended !== undefined)
            updateData.is_suspended = dto.is_suspended;
    ...
    async updateSuperadmin(id, dto, adminId) {
    ...
    async deactivateSuperadmin(id, adminId) {
        const user = await this.prisma.users.findUnique({ where: { id } });
        if (!user || user.role !== 'super_admin')
            throw new common_1.NotFoundException('Superadmin not found');
        const updated = await this.prisma.users.update({
    ...
    async getSuperadminDashboard() {
        const [totalUsers, totalTransactions, totalRevenue, activeTransactions, flaggedTx, supportTickets, pendingDisputes, pendingKyc] = await Promise.all([
            this.prisma.users.count({ where: { is_deleted: false } }),
        ...
        return {
            data: {
                ...
                pending_kyc: pendingKyc,
                system_health: 99,
                recent_activities: recentTx.map((tx) => ({
    ...
    async getComplianceReport(query) {
        ...
        const [kycVerified, kycPending, kycRejected, transactionsCompleted, transactionsFailed, suspiciousActivities,] = await Promise.all([
    ...
    async getSuperadminWallet(userId) {
        const user = await this.prisma.users.findUnique({
            where: { id: userId },
        ...
        const availableBalance = Math.max(0, Number(wallet.balance ?? 0) - Number(wallet.locked_balance ?? 0));
        return {
            data: {
    ...
    async superadminWithdraw(userId, dto) {
        const user = await this.prisma.users.findUnique({
            where: { id: userId },
            include: { wallets: { where: { is_active: true }, take: 1 } },
        });
