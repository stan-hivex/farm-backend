...
const paystack_webhook_util_1 = require("../payments/utils/paystack-webhook.util");
const deposit_utils_1 = require("../deposit/deposit.utils");
let WebhookService = WebhookService_1 = class WebhookService {
    constructor(prisma, depositService, withdrawService, websocket, notificationsService, cfg, paystackService, ivorypayService, webhookQueue, redisService) {
        this.prisma = prisma;
    ...
    async handlePaystackWebhook(payload, verified = false, rawBody, signature) {
        ...
        const paystackSecret = this.cfg.get('PAYSTACK_WEBHOOK_SECRET');
        if (!verified && paystackSecret && rawBody && signature) {
            try {
                const ok = (0, paystack_webhook_util_1.verifyPaystackSignature)(rawBody, signature, paystackSecret);
                verified = ok;
                if (!ok) {
                ...
                this.logger.error('Error verifying Paystack signature during processing', e);
            ...
            try {
                ...
                if (transaction) {
                    ...
                    if (expectedKobo === null || isNaN(webhookAmount)) {
                        ...
                        this.logger.warn(`Paystack amount difference for ${reference}: expected ${expectedKobo} kobo, received ${webhookAmount} kobo (diff: ${webhookAmount - expectedKobo} kobo)`);
                    }
                }
            }
            catch (e) {
                this.logger.error('Error validating Paystack amount', e);
        ...
        try {
            ...
            if (client) {
                try {
                    ...
                    this.logger.error('Failed to prepare Paystack webhook fallback', fallbackError);
                }
            }
            else {
                this.logger.error('Failed to enqueue Paystack webhook to Bull queue', e);
        ...
        if (!queued) {
            ...
            try {
                ...
                this.logger.error('Direct processing fallback failed for Paystack webhook', directError);
            }
        }
        else {
    ...
    async handleIvorypayWebhook(payload, verified = false) {
        ...
        const ivorySecret = this.cfg.get('IVORYPAY_WEBHOOK_SECRET');
        ...
        if (isSuccessEvent) {
            try {
                ...
                this.logger.error('Error validating Ivorypay amount', e);
        ...
        try {
            ...
            if (client) {
                try {
                    ...
                    this.logger.error('Failed to prepare Ivorypay webhook fallback', fallbackError);
                }
            }
            else {
                this.logger.error('Failed to enqueue Ivorypay webhook to Bull queue', e);
                await this.prisma.webhook_logs.update({ where: { id: log.id }, data: { status: 'failed', response: 'enqueue_error' } });
        ...
        if (!queued) {
            ...
            try {
                ...
                this.logger.error('Direct processing fallback failed for Ivorypay webhook', directError);
            }
        }
        else {
        ...
        if (eventId)
            await this.markProcessed('ivorypay', eventId);
        return { received: true };
    }
    async logWebhook(provider, event, payload, status = 'received', response) {
    ...
    async rejectWebhook(provider, event, payload, reason, meta) {
    ...
    verifyProviderPayload(provider, payload) {
    ...
    buildIvorypayReferenceCandidates(payload, metadata = {}) {
        const urlCandidates = [
    ...
    async verifyIvorypayWebhookTransaction(reference, providerReference, candidateRefs) {
        try {
    ...
    isIvorypayVerifiedAmountAcceptable(transaction, verifiedTransaction) {
        ...
        const verifiedUsd = Number(verifiedTransaction?.amount_usd ??
            verifiedTransaction?.data?.amount_usd ??
            verifiedTransaction?.amount ??
            verifiedTransaction?.data?.amount ??
            NaN);
        const verifiedFarm = Number(verifiedTransaction?.amount_farm ??
    ...
    async resolveIvorypayInternalReference(reference) {
        ...
        const transaction = await this.prisma.transactions.findFirst({
            where: {
                OR: [
                    ...
                    { metadata: { path: ['provider_ref'], equals: reference } },
                    { metadata: { path: ['provider_transaction_id'], equals: reference } },
                    { metadata: { path: ['provider_reference'], equals: reference } },
                    { metadata: { path: ['provider_payment_id'], equals: reference } },
                    { metadata: { path: ['provider_checkout_id'], equals: reference } },
                    { metadata: { path: ['payment_reference'], equals: reference } },
                    { metadata: { path: ['merchant_reference'], equals: reference } },
                    { metadata: { path: ['tx_ref'], equals: reference } },
                    { metadata: { path: ['trxref'], equals: reference } },
                    { metadata: { path: ['transaction_reference'], equals: reference } },
                    { metadata: { path: ['ivorypay_withdrawal_id'], equals: reference } },
                    { metadata: { path: ['withdrawal_id'], equals: reference } },
                    { metadata: { path: ['transfer_id'], equals: reference } },
                    { metadata: { path: ['payout_id'], equals: reference } },
                ],
            },
    ...
    async resolveIvorypayDepositAndTransaction(payload, fallbackReference) {
            ...
            const transaction = await this.prisma.transactions.findFirst({
                where: {
                    OR: [
                        ...
                        { metadata: { path: ['provider_ref'], equals: candidate } },
                        { metadata: { path: ['provider_transaction_id'], equals: candidate } },
                        { metadata: { path: ['provider_reference'], equals: candidate } },
                        { metadata: { path: ['provider_payment_id'], equals: candidate } },
                        { metadata: { path: ['provider_checkout_id'], equals: candidate } },
                        { metadata: { path: ['payment_reference'], equals: candidate } },
                        { metadata: { path: ['merchant_reference'], equals: candidate } },
                        { metadata: { path: ['tx_ref'], equals: candidate } },
                        { metadata: { path: ['trxref'], equals: candidate } },
                        { metadata: { path: ['transaction_reference'], equals: candidate } },
                        { metadata: { path: ['ivorypay_withdrawal_id'], equals: candidate } },
                        { metadata: { path: ['withdrawal_id'], equals: candidate } },
                        { metadata: { path: ['transfer_id'], equals: candidate } },
                        { metadata: { path: ['payout_id'], equals: candidate } },
                    ],
                },
            });
            let deposit = null;
    ...
    isIvorypaySuccessEvent(event, status) {
    ...
    isIvorypayFailureEvent(event, status) {
        const normalizedEvent = event?.toString()?.toLowerCase() ?? '';
        const normalizedStatus = status?.toString()?.toLowerCase() ?? '';
        const failureEvents = [
            'payment.failed',
    ...
    async fallbackAlert(provider, reason, payload) {
        try {
            ...
            try {
                ...
                this.logger.debug('Failed to emit websocket security alert', e);
            }
        }
        catch (e) {
            this.logger.error('Failed to create fallback alert', e);
        }
    }
    getEventId(provider, payload) {
        return (payload.id ?? payload.data?.id ?? payload.data?.reference ?? payload.reference ?? null);
    }
    async isReplay(provider, eventId) {
        try {
            const client = this.getRedisClient();
            if (!client)
                return false;
            const key = `${this.cfg.get('WEBHOOK_DEDUP_PREFIX', 'webhook:processed:')}${provider}:${eventId}`;
            const v = await client.get(key);
            return !!v;
        }
        catch (e) {
            this.logger.error('Failed to access Redis for webhook replay check', e);
            return false;
        }
    }
    async markProcessed(provider, eventId) {
        try {
            ...
            if (!client)
                return;
            const key = `${this.cfg.get('WEBHOOK_DEDUP_PREFIX', 'webhook:processed:')}${provider}:${eventId}`;
            const ttl = Number(this.cfg.get('WEBHOOK_DEDUP_TTL_MS', 0));
            if (ttl > 0) {
            ...
            this.logger.error('Failed to mark webhook processed in Redis', e);
        }
    }
    detectPotentialFraud(provider, payload) {
        const amount = this.normalizeAmount(payload?.data?.amount ?? payload?.amount ?? 0);
        const maxAmount = Number(this.cfg.get('FRAUD_MAX_WEBHOOK_AMOUNT', 5000000));
        if (amount > maxAmount) {
    ...
    async finalizeDeposit(reference) {
    ...
    async fixStuckDeposits() {
        ...
        for (const tx of stuck) {
            try {
                    ...
                    if (verifiedTransaction && provider === 'ivorypay') {
                        ...
                        if (lookupId && lookupId !== tx.transaction_reference) {
                            ...
                            try {
                                ...
                                this.logger.debug(`fixStuckDeposits: failed to persist verified provider id ${lookupId}`, updateErr);
                            }
                        }
                    }
                }
                catch (firstErr) {
                    this.logger.warn(`fixStuckDeposits: ${provider} verify lookup failed for ${tx.transaction_reference} using providerTransactionId=${providerTransactionId ?? 'internal'}`, firstErr);
                    continue;
                }
                const status = (verifiedTransaction?.status ?? '').toString().toLowerCase();
                if (['success', 'completed'].includes(status)) {
                ...
                this.logger.error(`fixStuckDeposits: failed to process ${tx.transaction_reference}`, err);
            }
        }
        await this.cleanupStaleFailedDeposits();
    }
    async cleanupStaleFailedDeposits() {
        ...
        if (staleTxs.length) {
            ...
            for (const tx of staleTxs) {
                try {
                    ...
                    this.logger.error(`cleanupStaleFailedDeposits: failed to delete stale deposit for transaction ${tx.transaction_reference}`, err);
                }
            }
        }
        ...
        for (const deposit of staleFailedDeposits) {
            try {
                ...
                this.logger.error(`cleanupStaleFailedDeposits: failed to delete stale FAILED deposit ${deposit.reference}`, err);
            }
        }
    }
    async finalizePendingDepositWithTransaction(reference, deposit, transaction) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const amount = this.normalizeAmount((0, deposit_utils_1.resolveDepositCreditAmount)(transaction, deposit));
    ...
    async creditPendingTransactionDeposit(reference) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const amt = this.normalizeAmount((0, deposit_utils_1.resolveDepositCreditAmount)(transaction));
        ...
        const previousBalance = result.previousBalance;
    ...
    async creditPendingDepositWithWallet(reference, deposit, transaction) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const previousBalance = this.normalizeAmount(Number(wallet.balance ?? 0));
            const creditAmount = this.normalizeAmount((0, deposit_utils_1.resolveDepositCreditAmount)(transaction, deposit));
            await tx.wallets.update({
                where: { id: wallet.id },
        ...
        const previousBalance = result.previousBalance;
        const amount = result.amount;
        const ledgerEntryId = result.ledgerEntryId;
        const notification = await this.notificationsService.sendNotification(deposit.userId, {
            type: 'deposit_completed',
            title: 'Deposit completed',
    ...
    async finalizeWithdrawal(reference, success, reason) {
    ...
    async completeTransactionWithdrawal(transaction) {
        if (!transaction || !transaction.id) {
            throw new common_1.BadRequestException('Invalid transaction for withdrawal completion');
        }
        if (transaction.status?.toLowerCase() === 'completed') {
            return true;
        }
        ...
        if (!wallet) {
            throw new common_1.BadRequestException(`Wallet not found for withdrawal transaction: ${transaction.id}`);
        }
    ...
    async failTransactionWithdrawal(transaction, reason) {
        if (!transaction || !transaction.id) {
            throw new common_1.BadRequestException('Invalid transaction for withdrawal failure handling');
        }
    ...
    async emitWithdrawalUpdate(reference, status) {
    ...
    async retryVerifyTransaction(reference) {
        const attempts = 36;
    ...
    getFarmAmountForCredit(transaction, deposit) {
        const metadata = transaction?.metadata ?? {};
        const depositMetadata = deposit?.metadata ?? {};
        const farmToUsdRate = Number(metadata?.farm_to_usd_rate ?? depositMetadata?.farm_to_usd_rate ?? this.cfg.get('IVORYPAY_FARM_TO_USD_RATE', '130')) || 130;
    ...
    async handlePaystackWebhookProcessing(payload) {
        const event = payload.event;
        const reference = payload.data?.reference;
        const status = payload.data?.status ?? payload.data?.gateway_response ?? payload.data?.failure_message ?? 'unknown';
        ...
        const lockTtl = Number(this.cfg.get('WEBHOOK_LOCK_TTL_MS', 60000));
        ...
        try {
            if (['charge.success', 'payment.success', 'transaction.success'].includes(event)) {
                ...
                if (transaction) {
                    ...
                    if (expectedKobo === null || isNaN(webhookAmount)) {
                        ...
                        this.logger.warn(`Paystack processing amount difference for ${reference}: expected ${expectedKobo} kobo, received ${webhookAmount} kobo (diff: ${webhookAmount - expectedKobo} kobo)`);
                    }
                }
                ...
                if (!verifiedTransaction || verifiedTransaction.status !== 'success') {
                    ...
                    setImmediate(() => {
                        this.retryVerifyTransaction(reference).catch((err) => this.logger.debug(`Background verify retries failed for ${reference}: ${err?.message ?? err}`));
                    });
                    return;
                }
                await this.depositService.finalizeSuccessfulDeposit(reference);
                ...
                const failureDetail = payload.data?.reason ||
            ...
            else if (['charge.failed', 'payment.failed', 'transaction.failed', 'charge.cancelled', 'payment.cancelled', 'transaction.cancelled',
            ...
            try {
                ...
                this.logger.debug('Failed to release lock for paystack webhook', e);
            }
        }
    }
    async acquireLock(key, ttlMs = 60000) {
        try {
            ...
            const res = await client.set(key, 'locked', 'NX', 'PX', ttlMs);
            return res === 'OK';
        }
        catch (e) {
            this.logger.error('Error acquiring webhook lock', e);
            return false;
        }
    }
    async releaseLock(key) {
        try {
            ...
            this.logger.debug('Error releasing webhook lock', e);
        }
    }
    async handleIvorypayWebhookProcessing(payload) {
        const event = payload.event ?? payload.status;
        ...
        const incomingProviderRef = payloadIdentifiers.transaction_id ??
        ...
        const incomingPayloadUpdate = {};
        const transactionMetadataUpdate = { ...transactionMetadata };
        if (deposit) {
            if (incomingProviderRef && deposit.providerRef !== incomingProviderRef)
                incomingPayloadUpdate.providerRef = incomingProviderRef;
            if (incomingProviderRef && deposit.providerTransactionId !== incomingProviderRef)
                incomingPayloadUpdate.providerTransactionId = incomingProviderRef;
            if (incomingProviderReference && deposit.providerReference !== incomingProviderReference)
                incomingPayloadUpdate.providerReference = incomingProviderReference;
            if (incomingCheckoutId && deposit.checkoutId !== incomingCheckoutId)
                incomingPayloadUpdate.checkoutId = incomingCheckoutId;
            if (incomingPaymentReference && deposit.paymentReference !== incomingPaymentReference)
                incomingPayloadUpdate.paymentReference = incomingPaymentReference;
            if (incomingMerchantReference && deposit.merchantReference !== incomingMerchantReference)
                incomingPayloadUpdate.merchantReference = incomingMerchantReference;
            if (payload && JSON.stringify(deposit.providerPayload) !== JSON.stringify(payload))
                incomingPayloadUpdate.providerPayload = payload;
        }
        if (transaction) {
            if (incomingProviderRef && transactionMetadata.provider_ref !== incomingProviderRef)
                transactionMetadataUpdate.provider_ref = incomingProviderRef;
            if (incomingProviderRef && transactionMetadata.provider_transaction_id !== incomingProviderRef)
                transactionMetadataUpdate.provider_transaction_id = incomingProviderRef;
            if (incomingProviderReference && transactionMetadata.provider_reference !== incomingProviderReference)
                transactionMetadataUpdate.provider_reference = incomingProviderReference;
            if (incomingCheckoutId && transactionMetadata.provider_checkout_id !== incomingCheckoutId)
                transactionMetadataUpdate.provider_checkout_id = incomingCheckoutId;
            if (incomingPaymentReference && transactionMetadata.provider_payment_id !== incomingPaymentReference)
                transactionMetadataUpdate.provider_payment_id = incomingPaymentReference;
            if (incomingMerchantReference && transactionMetadata.merchant_reference !== incomingMerchantReference)
                transactionMetadataUpdate.merchant_reference = incomingMerchantReference;
            if (payload && JSON.stringify(transactionMetadata.provider_payload) !== JSON.stringify(payload))
                transactionMetadataUpdate.provider_payload = payload;
        }
        ...
        const resolverProviderRef = providerRefFromMetadata ??
            incomingProviderRef ??
        ...
        try {
            ...
            if (isSuccessEvent) {
                ...
                const verifiedProviderId = verifiedTransaction.providerReference ??
                ...
                const providerTransactionId = verifiedProviderId ??
                ...
                const txHash = verifiedTransaction?.tx_hash ??
                ...
                if (transaction) {
                    ...
                    if (expectedUsd !== null && isFinite(expectedUsd) && !isNaN(webhookAmount)) {
                        if (Math.abs(webhookAmount - expectedUsd) > 0.01) {
                            ...
                            throw new common_1.BadRequestException(`Amount mismatch: expected ${expectedUsd}, got ${webhookAmount}`);
                        }
                    }
            ...
            if (transaction?.transaction_type === 'withdrawal' || ['withdrawal.success', 'transfer.success', 'payout.success', 'withdrawal.completed', 'transfer.completed', 'payout.completed'].includes(event)) {
                ...
                await this.depositService.failDeposit(resolvedReference, payload.data?.reason || payload.data?.message || payload.message || 'Payment failed or cancelled');
            }
        }
        catch (error) {
            ...
            try {
                ...
                this.logger.debug('Failed to release lock for ivorypay webhook', e);
            }
        }
    }
};
...
], WebhookService.prototype, "fixStuckDeposits", null);
exports.WebhookService = WebhookService = WebhookService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(8, (0, bull_1.InjectQueue)(constants_1.QUEUES.WEBHOOKS)),
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        ...
        ivorypay_service_1.IvorypayService, Object, redis_service_1.RedisService])
], WebhookService);
