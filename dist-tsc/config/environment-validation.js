...
function validateSecurityEnvironment() {
    ...
    const validateSecret = (secretName, minLength = 32) => {
        ...
        throw new Error(`Production security validation failed: ${secretName} must be set to a strong value (minimum ${minLength} characters).`);
    };
    if (isProduction) {
        ...
        if (!process.env.JWT_ACCESS_SECRET) {
            logger.warn('⚠️ JWT_ACCESS_SECRET not set - using development default. DO NOT use in production!');
        }
        if (!process.env.QR_HMAC_SECRET) {
            logger.warn('⚠️ QR_HMAC_SECRET not set - using development default. DO NOT use in production!');
        }
        if (!process.env.FIELD_ENCRYPTION_KEY) {
            logger.warn('⚠️ FIELD_ENCRYPTION_KEY not set - using temporary development key. DO NOT use in production!');
        }
    }
}
